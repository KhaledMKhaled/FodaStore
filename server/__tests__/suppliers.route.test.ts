...
test("GET /api/suppliers excludes hidden suppliers by default", async () => {
  suppliersFixture = [
    buildSupplier({ id: 1, name: "Visible Supplier" }),
    buildSupplier({ id: 2, name: "Hidden Supplier", isHidden: true }),
  ];
  ...
  assert.equal(body[0].name, "Visible Supplier");
});

test("GET /api/suppliers includes hidden suppliers when includeHidden=true", async () => {
  suppliersFixture = [
    buildSupplier({ id: 1, name: "Visible Supplier" }),
    buildSupplier({ id: 2, name: "Hidden Supplier", isHidden: true }),
  ];
  ...
  const response = await fetch(
    `http://127.0.0.1:${port}/api/suppliers?includeHidden=true`,
  );
  const body = await response.json();
  await close();
  ...
  assert.deepEqual(
    ...
    ["Visible Supplier", "Hidden Supplier"],
  );
});
